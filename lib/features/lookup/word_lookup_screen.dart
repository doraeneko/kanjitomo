...
class _WordLookupScreenState extends State<WordLookupScreen> {
  ...
  Future<void> _addToReview(WordEntry entry) async {
    ...
    for (final char in selectedChars) {
      ...
      // If char not in custom character set, add it.
      final scope = widget.deps.studyScope.scope.value;
      if (!scope.customCharacters.contains(char)) {
        final newChars = {...scope.customCharacters, char};
        await widget.deps.studyScope.update(
          scope.copyWith(customCharacters: newChars),
        );
      }
    }
  ...
  Widget build(BuildContext context) {
    final l = AppLocalizations.of(context)!;
    return Scaffold(
      appBar: AppBar(title: Text(l.wordLookupTitle)),
